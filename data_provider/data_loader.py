...
class Dataset_ApneaECG(Dataset):
    """Loads a single column CSV of ECG samples + datetime"""

    def __init__(self, root_path, flag='train', size=None, features='S', data_path=None, target='ECG', scale=False, timeenc=0, freq='10ms', **kwargs):
        # unpack sizes
    ...
    def __len__(self):
        ...
        total = len(self.series) - self.seq_len - self.pred_len + 1
        return max(total, 0)
    ...
    def __getitem__(self, idx):
        ...
        Returns:
          X: [seq_len]      past ECG
          y: [pred_len]     future ECG to forecast
        """
        start = idx
        mid = idx + self.seq_len
        end = mid + self.pred_len

        X = self.series[start:mid]
        y = self.series[mid:end]

        # shape → (seq_len, 1) and (pred_len, 1)
