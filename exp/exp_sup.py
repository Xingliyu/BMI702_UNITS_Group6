...
class Exp_All_Task(object):
        ...
        if self.args.pretrained_weight is not None:
            ...
            if 'pretrain_checkpoint.pth' in pretrain_weight_path:
                state_dict = torch.load(
                    pretrain_weight_path, weights_only=False, map_location='cpu')['student']
                ckpt = {}
                for k, v in state_dict.items():
                    if not ('cls_prompts' in k):
                        ckpt[k] = v
            else:
                ckpt = torch.load(pretrain_weight_path, weights_only=False, map_location='cpu')
    ...
    def test_long_term_forecast(self, setting, test_data, test_loader, data_task_name, task_id):
        ...
        # np.save(os.path.join(self.path, 'preds.npy'), preds)
        # np.save(os.path.join(self.path, 'trues.npy'), trues) 
